...
class ManagerLevel {
    ...
    void update(ManagerGame _gm, ManagerSound _sm){
        if (!_gm.isWin() && !_gm.isLoose()) {
            ...
            if (ball.getStateY()==GOStateVertical.UP && ball.getRect().top>=gameField.top)
                ...
                else ball.switchDirectionY();
            }
            if (ball.isBoosted())ball.slowdown();
    ...
    public GOBlock getBlock(int y,int x) {return blocks[y][x];}

    GORacket getRacket(){return racket;}
    GOBall getBall(){return ball;}
